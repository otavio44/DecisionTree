...
char *inst[TOTALINST] = {
    ...
    "ARZV", /* Armazena no vetor */
    "CRVV", /* Carrega valor do vetor */

    "FIMP"  /* Fim do programa  - OBS: Tem que ser a ultima da lista*/
};

enum codinst {
    CRVG, CRCT, SOMA, SUBT, MULT, DIVI, CMIG,
    CMMA, CMME, CONJ, DISJ, NEGA, ARZG, DSVS, 
    DSVF, NADA, ESCR, LEIA, INPP, AMEM, DMEM, 
    ARZV, CRVV, FIMP
};
    ...
    *nro = 0;
    while (!feof(arq)) {
        ...
        if (linha[0] == 'L') /*--- tipo 1 ---*/ {
            ...
            // FIMP = tem que ser o ultimo da lista
    ...
    while (1) {
        switch (P[i].i) {
            ...
            case DMEM: s = s - P[i].o;
                i++;
                break;
            default: printf("Instrucao MVS desconhecida[%d,%d]!\n",i, P[i].i);
                exit(0);
        }
    }
}
